...
func init() {
	...
	utilruntime.Must(keycloakv1.AddToScheme(scheme))
...
func main() {
	...
	zerologInstance := zerolog.New(os.Stdout).With().Timestamp().Caller().Logger()
	level, err := zerolog.ParseLevel(logLevel)
	if err != nil {
		zerologInstance.Warn().Str("provided", logLevel).Msg("Invalid log level, defaulting to info")
		level = zerolog.InfoLevel
	}
	zerologInstance = zerologInstance.Level(level)

	// Create logr logger from zerolog
	logger := zerologr.New(&zerologInstance)
	ctrl.SetLogger(logger)

	// Redirect klog to our logger (this handles leader election and other k8s client-go logs)
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "1cf2fb01.pewty.fr",
	...
	if keycloakURL == "" || keycloakUser == "" || keycloakPass == "" || keycloakRealm == "" {
	...
	token, err := keycloakClient.LoginClient(ctx, keycloakUser, keycloakPass, keycloakRealm)
	if err != nil {
